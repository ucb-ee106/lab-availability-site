...
# ---------------------------------------------------------------------------
# get_station_states (now in lab_utils, tested via check_notifications usage)
# ---------------------------------------------------------------------------
...
class TestGetCurrentStates:
    def test_reads_station_status(self, tmp_path):
        ...
        with mock.patch.object(lab_utils, 'STATION_STATUS_CSV_PATH', str(csv_file)):
    ...
    def test_missing_file_returns_empty(self):
        with mock.patch.object(lab_utils, 'STATION_STATUS_CSV_PATH', '/nonexistent'):
            states = lab_utils.get_station_states()
        assert states == {}


# ---------------------------------------------------------------------------
# get/save previous states (now in lab_utils)
# ---------------------------------------------------------------------------

class TestPreviousStates:
    def test_roundtrip(self, tmp_path):
        states_file = tmp_path / "previous.json"
        with mock.patch.object(lab_utils, 'PREVIOUS_STATES_PATH', str(states_file)):
            lab_utils.save_states({1: True, 2: False})
            loaded = lab_utils.get_previous_states()
        assert loaded == {1: True, 2: False}

    def test_missing_file_returns_empty(self):
        with mock.patch.object(lab_utils, 'PREVIOUS_STATES_PATH', '/nonexistent'):
            assert lab_utils.get_previous_states() == {}


# ---------------------------------------------------------------------------
# get_first_in_queue (now in lab_utils)
# ---------------------------------------------------------------------------
...
class TestGetFirstInQueue:
    def test_returns_first_person(self, tmp_path):
        ...
        with mock.patch.object(lab_utils, 'QUEUE_TURTLEBOT_CSV_PATH', str(csv_file)):
            person = lab_utils.get_first_in_queue('turtlebot')
    ...
    def test_missing_file_returns_none(self):
        with mock.patch.object(lab_utils, 'QUEUE_TURTLEBOT_CSV_PATH', '/nonexistent'):
            person = lab_utils.get_first_in_queue('turtlebot')
        assert person is None
...
class TestCheckExpiredClaims:
    def test_keeps_active_claims(self, tmp_path):
        ...
        with mock.patch.object(lab_utils, 'PENDING_CLAIMS_CSV_PATH', str(claims_path)):
    ...
    def test_removes_expired_unconfirmed(self, tmp_path):
        ...
        with mock.patch.object(lab_utils, 'PENDING_CLAIMS_CSV_PATH', str(claims_path)), \
             mock.patch.object(lab_utils, 'QUEUE_TURTLEBOT_CSV_PATH', str(queue_path)), \
